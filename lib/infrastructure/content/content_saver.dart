...
class ContentSaver {
  ...
  Future<_ContentFiles> _entriesByFile(
    ...
    return _ContentFiles(files: files, array: array, key: key);
  }
}
...
class _ContentFiles {
  ...
  Set<String> merge({
    ...
    for (final entry in entries.values) {
      ...
      if (position >= 0 && _same(existing[position], entry)) continue;
      ...
      if (position >= 0) {
        ...
        updated[position] = <String, dynamic>{...existing[position], ...entry};
      } else {
        updated.add(entry);
      }
      json[array] = updated;
